    ...
    for (int row = 0; row < 9; ++row)
    {
        for (int col = 0; col < 9; ++col)
        {
            if (!_row_block[row].isValid() || 
                ...
                printf("%d %d %d %d %d\n",row,col,_row_block[row].isValid(),_column_block[col].isValid(), _xy_block[row / 3][col / 3].isValid());
                return false;	
            }
...
void swaprow(char mpp[][10],int i, int j){
    for(int k=0;k<9;k++){
        std::swap(mpp[i][k],mpp[j][k]);
    }
}
void swapcol(char mpp[][10],int i, int j){
    for(int k=0;k<9;k++){
        std::swap(mpp[k][i],mpp[k][j]);
    }
}
void CScene::generate()
{
    ...
    for(int i=0;i<3;i++){
        int w=random(0,2);
        int r1=(w+1)%3;
        swaprow(map_pattern,w+i*3,r1+i*3);
    }
     for(int i=0;i<3;i++){
        int w=random(0,2);
        int r1=(w+1)%3;
        swapcol(map_pattern,w+i*3,r1+i*3);
    }
    ...
    std::unordered_map<int, bool> vis_map;
    for (int i = 1; i <= 9; ++i)
    {
        int s = random(1,99);
    	while(vis_map[s]==true){
            s=random(1,99);
        }
        vis_map[s]=true;
        int r = random(0, v.size() - 1);
